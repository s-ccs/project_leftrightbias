...
We find an inverse quadratic relationship between the left-right bias and power (see @fig:power). Meaning, the stronger the left-right bias, the smaller is the power. The maximal statistical power can be found at no-bias (a "50%" bias). The relationship can be translated to gains in power ($"power-at-maximum" / "power-at-bias"*100$), which ranges from being small (\~5% power gain with little left bias, 65% bias) to decent (\~30%, at\~80% bias) to large (\~300% at \~90%) at log-odds of 0.5, with even higher values for slightly smaller log-odds or more extreme biases.

#figure(
    image("2026-02-03_sim-results.svg",width:100%),
    caption:[
        A) Simulations at different left/right biases (x-axis). To follow the example of the text, the orange results would be average responses to a happy face on the right, blue to one on the left, with a task to identify the happy face. Already here one can appreciate the larger difference in %-points at a 50:50 bias, compared to more extreme bias values. B) Left-right bias against statistical power while varying the effect size. At log-odds of 0.5 we achieve 80% power when no left/right bias exists. If bias increases to either side, we can see a drastic decrease in statistical power. C) This decrease in power can be re-interpreted as an increase in power, if one would eliminate the left/right bias (log-y-axis-scale for visualization purposes). E.g. for a bias of 0.1 (90% to the left), one would expect a 3x power increase if the bias could be changed to 0.5 (50% to the left).
    ]
) <fig:power>
