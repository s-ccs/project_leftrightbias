...
== Participants
A total of 23 participants (age range 20–60, (FIXME(JO): add mean age) 3 female, 20 male, 0 diverse) were included in the study. All participants were raised in a culture where the Latin script is utilized and text is written from left to right.
...
We replicated the well-known leftward spatial bias when two images are displayed simultaneously and also demonstrated an equally strong top bias. We further demonstrated that diagonal positioning of stimuli in a 2/7 o'clock configuration (FIXME(JO): is it 2/7 or 1/7? needs to be fixed below too) can eliminate spatial bias because the left and top bias cancel out in this arrangement. These results are relevant for studies in which multiple stimuli are presented in different locations of the visual field, ranging from two-choice tasks to visual search. Even for covert tasks, or when the direction of the first eye movement is itself irrelevant, strong scanning biases can reduce the dynamic range available for experimental manipulation and affect sensitivity measurements. Moreover, these results indicate that our understanding of the mechanisms behind spatial biases still remains incomplete.

The relation of our findings to three studies should be highlighted here: In principle, we are conceptually replicating some of the findings of #cite(<schutz_interindividual_2014>,form:"prose"). They used only two stimuli at a time and found the same overall top-left bias as seen in our results. #cite(<durgin_upper-left_2008>, form:"prose") found a similar top-left bias in a visual search task. #cite(<churches_perceptual_2017>, form:"prose") found, in an attention task without eye-movements, horizontal and vertical biases (FIXME(TM) I don’t understand the last bit of the sentence). In summary, different studies using displays with discrete stimuli show a top left bias, suggesting that this is a robust phenomena reflecting a more general perceptual/attentional mechanism (FIXME(TM) generality -> general sounds tautological, rephrase general -> low-level?).
